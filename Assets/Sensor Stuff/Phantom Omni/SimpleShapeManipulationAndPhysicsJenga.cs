...
public class SimpleShapeManipulationAndPhysicsJenga : HapticClassScript {
	...
	void Awake()
	{
		...
		//workSpaceObj = GameObject.Find("WorkspaceCube(Clone)");
	...
	void Start()
	{
		...
		Debug.Log("AWAKE");
		hapticCursor =  GameObject.Find("Omni(Clone)/Pointer");
		myHapticCamera = GameObject.Find("Omni(Clone)/Haptic Camera");
